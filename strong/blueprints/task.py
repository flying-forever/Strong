    ...
    if form.validate_on_submit():
        ...
        def task_split(task: Task):
            ...
            if s.day != f.day:
                ...
                task_copy(task=task, use_minute=um - more, describe=form.describe.data+'(截断)', tfc=e, is_finish=True) # 直接拷贝时提交
                return more

        # 4 跨过0点 - 任务截断成两个（没加经验 - 修改旧任务跨过了0点也截断
        more_minute =  task_split(task=task)
        # 1 修改旧任务 --> 写入表单数据
        if task.is_finish:
        ...
        return form_commit(task=task, use_minute=more_minute)
